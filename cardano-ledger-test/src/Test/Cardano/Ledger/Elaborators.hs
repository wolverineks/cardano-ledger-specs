...
lookupModelValue path scriptAction = \case
  ModelValue_Reward maddr -> do
    allRewards <- observeRewards path <$> State.get
...
mkTxOut ::
  ...
  m (Core.TxOut era)
mkTxOut txid mutxoId (ModelTxOut mAddr (ModelValue mValue) mDat) = (=<<) Except.liftEither $
  State.state $
    State.runState $
      Except.runExceptT $ do
        addr <- zoom _2 $ getAddrFor (Proxy :: Proxy era) mAddr
        val :: Core.Value era <- either (Except.throwError . ElaborateBlockError_TxValue @era) pure =<< evalModelValue (lookupModelValue txid noScriptAction) mValue
  ...
  elaborateBlock globals =
    State.runState . Except.runExceptT . \case
      ModelBlock mslot mtxSeq -> do
        ...
        txSeq ::
          [Core.Tx era] <-
          ifor mtxSeq $ \txi tx -> Except.ExceptT $ State.state $ elaborateTx (Proxy :: Proxy era) (currentEpoch, mslot, txi) globals ttl tx
      ...
      (txBodyArguments, txAccum) <- Writer.runWriterT $ do
        ...
        fee :: Coin <-
          liftEvalModelValue ElaborateBlockError_Fee $
            evalModelValue (lookupModelValue mtxid noScriptAction) mtxFee
              ...
              SupportsMint m' ->
                fmap SupportsMint $
                  liftEvalModelValue ElaborateBlockError_TxValue $
                    evalModelValue (lookupModelValue mtxid (tellMintWitness @era)) (unModelValue m')
...
-- ModelTx's don't carry around an identifier; since they don't need to be
-- referred to, in the model proper, but it turns out to be useful to report the
-- location of an error.
type ModelTxId = (EpochNo, SlotNo, Int)
