...
{-# OPTIONS_GHC -Wno-orphans #-} -- HeapWords for Array and PrimArray
...
import Data.Primitive.PrimArray(PrimArray,indexPrimArray,sizeofPrimArray,primArrayFromList,primArrayToList)
...
insert k v (CompactMap s active1 m1) = CompactMap s active2 (m2 <> m1)
  where m2 = Delta(Map.singleton k (Edit v))
...
delete k (CompactMap s active1 m1) = CompactMap s active2 (m2 <> m1)
  where m2 = Delta(Map.singleton k Delete)
        active2 = applyMessages active1 m2
...
getValues (Par2 _n _keys vs _diff) = foldr' accum [] vs
   where accum bytes ans = (unsafeDeserialize' bytes :: [v]) ++ ans
