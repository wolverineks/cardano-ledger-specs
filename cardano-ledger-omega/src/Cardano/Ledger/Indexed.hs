...
module Cardano.Ledger.Indexed where
...
import Test.Cardano.Crypto.Utils (mkKeyPair)
...
{-
instance CC.Crypto c => Fixed (VKey kr c) where
    unique n = vk where (KeyPair vk _) = unique @(KeyPair kr c) n

instance CC.Crypto c => Fixed (SKey kr c) where
    unique n = (SKey sk) where (KeyPair _ sk) = unique @(KeyPair kr c) n

instance CC.Crypto c => Fixed (KeyHash kr c) where
    unique n = hashKey (unique @(VKey kr c) n)

vkey9 :: VKey 'Payment Mock
vkey9 = pickCbyCrypto @(VKey 'Payment) 9 Mock
-}
